...
/**
 * Homologador de Nóminas - Lógica de Aplicación
 * Procesa archivos Excel/CSV en local y adapta la estructura según reglas predefinidas.
 */

// Estructura de Columnas Destino requerida por el SaaS
const TARGET_COLUMNS = [
    ...
    { key: 'tipo_nomina', name: 'Tipo de Nómina', required: true, desc: 'Tipo de nómina (ej. Normal, Adicional).', synonyms: ['tipo de nomina', 'tipo de nómina', 'tipo nomina', 'tipo_nomina', 'tipo_de_nomina', 'clase_nomina'] },
...
let appState = {
    ...
    sheetNames: [],
    selectedSheet: '',
    rawHeaders: [], // Encabezados detectados en el archivo origen
    rawRows: [],    // Filas de datos del archivo origen (sin procesar)
    columnMappings: {}, // Mapeo { targetKey: sourceHeader }
    combineNames: false,
    combineSurnamesList: [],
    combineNamesList: [],
    detectedSplitNames: false, // Indica si se detectó nombre/apellido separado
    quincenaRule: 'quincenal', // 'quincenal' o 'mensual'
    defaultTipoNomina: 'Normal', // Tipo de nómina por defecto
    ...
    monthColumn: '',
    yearColumn: '',
    dayColumn: '', // Columna de día (opcional si se selecciona "No aplica")
    monthYearDayRule: 'last',
    detectedSplitMonthYear: false,
    suggestedMonthColumn: '',
    suggestedYearColumn: '',
    suggestedDayColumn: '', // Columna sugerida de día
    transformedData: [],
    validationErrors: [],
    genericUnifications: {} // Mapeo { targetKey: { columns: [], separator: ' ' } }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Vincular Eventos de Paso 1 (Carga)
    ...
    // Vincular Eventos de Paso 2 (Mapeo)
    document.getElementById('back-to-1-btn').addEventListener('click', () => setStep(1));
    document.getElementById('go-to-3-btn').addEventListener('click', validateAndGoToStep3);

    // Vincular Eventos de Paso 3 (Vista Previa y Descarga)
    document.getElementById('back-to-2-btn').addEventListener('click', () => setStep(2));
    document.getElementById('download-excel-btn').addEventListener('click', downloadTransformedExcel);
});

// ==========================================
// CONTROL DEL FLUJO (WIZARD)
// ==========================================
function setStep(stepNum) {
    appState.currentStep = stepNum;
    
    // Actualizar Clases Activas de Pasos en UI
    for (let i = 1; i <= 3; i++) {
    ...
    // Mapeo: Paso 1 -> 16.6%, Paso 2 -> 50%, Paso 3 -> 100%
    const progressPercent = stepNum === 1 ? 16.6 : (stepNum === 2 ? 50 : 100);
    progressFill.style.width = `${progressPercent}%`;
    
    // Re-renderizar íconos en el panel activo
    lucide.createIcons();
}
...
// ==========================================
// PASO 1: LECTURA Y PROCESAMIENTO DEL ARCHIVO
// ==========================================
...
function processFile(file) {
    ...
    showLoading('Leyendo y analizando archivo de nómina...');
    ...
    reader.onload = function(e) {
        try {
            ...
            if (appState.sheetNames.length > 1) {
                ...
                // Procesar la única pestaña
                appState.selectedSheet = appState.sheetNames[0];
                loadSheetData(appState.selectedSheet);
            }
        } catch (err) {
            console.error(err);
            alert('Error al leer el archivo. Asegúrate de que sea un archivo de Excel o CSV válido.');
            hideLoading();
        }
    };
...
function renderSheetSelector() {
    ...
    appState.sheetNames.forEach(sheetName => {
        ...
        btn.addEventListener('click', () => {
            ...
            showLoading(`Cargando pestaña "${sheetName}"...`);
            setTimeout(() => loadSheetData(sheetName), 100);
        });
...
function loadSheetData(sheetName) {
    try {
        ...
        // Convertir hoja a matriz de matrices (row-by-row raw arrays)
        ...
        // Buscamos la primera fila en las primeras 10 filas que tenga el mayor número de columnas no vacías
        ...
        // Obtener los nombres de las columnas (normalizados para visualización)
        ...
        for (let i = headerRowIndex + 1; i < rawMatrix.length; i++) {
            ...
            if (!isEmptyRow) {
                // Alinear datos con los encabezados
                const rowObj = {};
        ...
        // Mostrar animación de analizando estructura
        showLoading('Analizando la estructura del archivo y auto-mapeando...');
        
        setTimeout(() => {
            // Ejecutar Auto-Mapeo Inicial
            runAutoMapping();
            
            // Silenciosamente activar las unificaciones detectadas
            ...
            renderMappingUI();
            setStep(2);
        }, 1500); // 1.5 segundos de animación de análisis
    } catch (err) {
        console.error(err);
...
function runAutoMapping() {
    ...
    TARGET_COLUMNS.forEach(target => {
        // Buscar coincidencia exacta o por sinónimo
        let matchedHeader = '';
        
        for (let header of appState.rawHeaders) {
            const normalizedHeader = cleanString(header);
            
            // Buscar en sinónimos del campo destino
        ...
        if (matchedHeader) {
            ...
            // Si es un campo opcional o especial, inicializar como vacío
            appState.columnMappings[target.key] = '';
        }
    });

    // Validar si es altamente probable que el nombre venga separado
    detectSplitNames();
    // Validar si vienen mes y año separados
...
function cleanString(str) {
    ...
    return str.toString()
        ...
        .replace(/[^a-z0-9]/g, " ")     // Reemplazar caracteres especiales con espacio
        .replace(/\s+/g, " ")           // Unificar espacios múltiples
        .trim();
}

function detectSplitNames() {
    // Buscar si hay columnas específicas de apellidos y nombres separadas
    ...
    appState.rawHeaders.forEach(header => {
        ...
        // Evitar falsos positivos como "nombre completo"
    ...
    // Si encontramos columnas candidatas a apellidos Y nombres, registramos la detección independientemente de si se mapeó automáticamente Nombre Completo
    if (matchedSurnames.length > 0 || matchedNames.length > 0) {
        appState.detectedSplitNames = true;
        appState.combineNames = false; // Se activará por confirmación del modal o manualmente
...
function detectSplitMonthYear() {
    ...
    if (matchedMonth && matchedYear) {
        ...
        // No activar de inmediato, pero pre-configurar las columnas para si deciden unificar
...
// ==========================================
// PASO 2: RENDERING & CONTROL DE MAPEO DE COLUMNAS
// ==========================================
// Helpers para obtener vista previa de datos mapeados
// Helpers para resolver valores de mapeo normales o unificados de una fila
...
function updateRowPreview(targetKey) {
    ...
    if (targetKey === 'nombre_completo' && appState.combineNames) {
        ...
        // Siempre mostrar el valor por defecto/mapeado de tipo_nomina
        previewText = getFieldValuePreview(targetKey);
        isMappedOrUnified = true;
    } else if (targetKey === 'quincena') {
        // Siempre mostrar la regla activa de quincena
        previewText = getFieldValuePreview(targetKey);
        isMappedOrUnified = true;
...
function renderMappingUI() {
    ...
    TARGET_COLUMNS.forEach(target => {
        ...
        // Contenedor de acciones y previsualización (Columna 3 de la grilla)
        ...
        // 2. Icono pequeño para seleccionar las columnas (Botón configurar unificación para TODAS las filas)
        ...
        let unifyTitle = `Personalizar columna: ${target.name}`;
        
        btnUnify.title = unifyTitle;
        btnUnify.innerHTML = `<i data-lucide="${iconName}"></i>`;
        ...
        // 3. Icono de Alerta / Éxito para TODAS las filas
        ...
        if (isMapped) {
            ...
            if (target.key === 'nombre_completo' && appState.detectedSplitNames) {
                icon.title = 'Nombres y Apellidos separados en el archivo. ¡Pulsa el botón de personalizar columna para combinarlos!';
                ...
                icon.title = 'Mes y Año separados en el archivo. ¡Pulsa el botón de personalizar columna para combinarlos!';
                icon.innerHTML = `<i data-lucide="alert-circle" class="text-warning"></i>`;
                icon.addEventListener('click', openCombineMonthYearModal);
            } else if (target.key === 'tipo_nomina') {
                icon.title = 'No se mapeó el Tipo de Nómina. ¡Pulsa para seleccionar el valor por defecto!';
                icon.innerHTML = `<i data-lucide="alert-circle" class="text-warning"></i>`;
                icon.addEventListener('click', openTipoNominaCustomizationModal);
            } else if (target.key === 'quincena') {
                icon.title = 'No se mapeó la Quincena. ¡Pulsa para seleccionar la regla por defecto!';
                icon.innerHTML = `<i data-lucide="alert-circle" class="text-warning"></i>`;
        ...
        // 4. Previsualización de un único dato de la columna mapeada
...
// Modal interactivo para configurar la unificación de nombres/apellidos
function openCombineNamesModal() {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay payroll-adapter-scope';
    ...
    // Contenedor principal de selección (Lista vertical)
    const selectionContainer = document.createElement('div');
    selectionContainer.className = 'form-group';
    selectionContainer.innerHTML = '<label style="margin-bottom: 0.75rem; display: block; font-weight: 600;">Selecciona las columnas en el orden en que deseas unificar el nombre completo:</label>';
...
// Modal interactivo para configurar la unificación de mes y año
function openCombineMonthYearModal() {
    ...
    // 1. Contenedor de selección de columnas (Lista vertical)
    const selectionContainer = document.createElement('div');
    selectionContainer.className = 'form-group';
    selectionContainer.innerHTML = '<label style="margin-bottom: 0.75rem; display: block; font-weight: 600;">Selecciona las columnas en orden: Día (1), Mes (2) y Año (3) [o solo Mes (1) y Año (2)]:</label>';
    ...
    function renderColumns() {
        ...
        appState.rawHeaders.forEach(headerName => {
            ...
            input.addEventListener('change', () => {
                ...
                // Si seleccionan 3 columnas, forzar la regla a "none"
                if (tempOrderedList.length === 3) {
                    selectDay.value = 'none';
                    tempDayRule = 'none';
                } else if (tempOrderedList.length === 2 && tempDayRule === 'none') {
                    // Si bajan a 2 columnas y estaba en "none", cambiar a "last" por defecto
    ...
    // 2. Selector de regla de día a asignar
    const formDay = document.createElement('div');
    formDay.className = 'form-group';
    formDay.style.marginTop = '0.5rem';
    formDay.innerHTML = '<label style="font-weight: 600;">Día del Mes a Asignar</label>';
...
// Modal interactivo genérico para configurar la unificación de columnas para cualquier campo
function openGenericUnificationModal(targetKey) {
    ...
    const selectionContainer = document.createElement('div');
    selectionContainer.className = 'form-group';
    selectionContainer.innerHTML = '<label style="margin-bottom: 0.75rem; display: block; font-weight: 600;">Selecciona las columnas del archivo origen en el orden en que deseas unificarlas:</label>';
    ...
    formSeparator.innerHTML = '<label style="font-weight: 600;">Separador de Columnas</label>';
...
// Modal para personalizar Tipo de Nómina por defecto
function openTipoNominaCustomizationModal() {
    ...
    body.className = 'modal-body';
    body.innerHTML = `
        <p>No se identificó la columna de Tipo de Nómina en el archivo origen. Selecciona el tipo de nómina por defecto a aplicar para todas las filas de este cargue:</p>
...
// Modal para personalizar la regla de Quincena por defecto
function openQuincenaCustomizationModal() {
    ...
    body.className = 'modal-body';
    body.innerHTML = `
        <p>No se identificó la columna de Quincena en el archivo origen. Selecciona la regla de cálculo del periodo de pago:</p>
        <div class="options-group" style="grid-template-columns: 1fr; gap: 1rem; width: 100%;">
            <label class="option-btn" style="width: 100%; display: flex; gap: 0.75rem; align-items: flex-start; padding: 1rem; background: rgba(0,0,0,0.15); border: 1px solid var(--border-color); border-radius: var(--radius-md); cursor: pointer;">
                <input type="radio" name="modal-quincena-rule" value="quincenal" ${appState.quincenaRule === 'quincenal' ? 'checked' : ''}>
                <div class="option-content">
                    <span class="option-title" style="font-weight: 600; color: white;">Nómina Quincenal</span>
                    <span class="option-desc" style="font-size: 0.75rem; color: var(--text-muted); display: block; margin-top: 0.25rem;">Día 1-15 se asigna como "1Q". Día 16-fin de mes se asigna como "2Q" (usando Fecha de Acumulado).</span>
                </div>
            </label>
            <label class="option-btn" style="width: 100%; display: flex; gap: 0.75rem; align-items: flex-start; padding: 1rem; background: rgba(0,0,0,0.15); border: 1px solid var(--border-color); border-radius: var(--radius-md); cursor: pointer;">
                <input type="radio" name="modal-quincena-rule" value="mensual" ${appState.quincenaRule === 'mensual' ? 'checked' : ''}>
                <div class="option-content">
                    <span class="option-title" style="font-weight: 600; color: white;">Nómina Mensual</span>
                    <span class="option-desc" style="font-size: 0.75rem; color: var(--text-muted); display: block; margin-top: 0.25rem;">Toda la nómina se asigna con el valor de periodo "Mensual".</span>
                </div>
            </label>
        </div>
    `;
...
// Modal informativo sobre la regla automática de Naturaleza
function openNaturalezaInfoModal() {
    ...
    if (isMappedFromFile) {
        body.innerHTML = `
            <p>La columna <strong>${mappedHeader}</strong> del archivo origen está siendo usada para determinar la naturaleza de cada concepto.</p>
            <p>Si el valor en esa columna no se reconoce como <strong>INGRESO</strong> o <strong>DESCUENTO</strong>, se aplicará automáticamente la regla por signo del <strong>Valor</strong>:</p>
            <div style="display:flex; gap:1rem; margin-top:0.5rem;">
                <div style="flex:1; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.3); border-radius:var(--radius-md); padding:1rem; text-align:center;">
                    <i data-lucide="trending-up" style="color:var(--color-success); width:28px; height:28px; margin-bottom:0.5rem;"></i>
                    <div style="font-weight:700; color:var(--color-success); font-size:1rem;">INGRESO</div>
                    <div style="font-size:0.8rem; color:var(--text-muted); margin-top:0.25rem;">Valor ≥ 0</div>
                </div>
                <div style="flex:1; background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.3); border-radius:var(--radius-md); padding:1rem; text-align:center;">
                    <i data-lucide="trending-down" style="color:var(--color-danger); width:28px; height:28px; margin-bottom:0.5rem;"></i>
                    <div style="font-weight:700; color:var(--color-danger); font-size:1rem;">DESCUENTO</div>
                    <div style="font-size:0.8rem; color:var(--text-muted); margin-top:0.25rem;">Valor < 0</div>
                </div>
            </div>
        `;
        ...
        body.innerHTML = `
            <p>No se encontró la columna de Naturaleza en el archivo origen. Se aplicará automáticamente la siguiente regla basada en el <strong>Valor</strong> de cada fila:</p>
            <div style="display:flex; gap:1rem; margin-top:0.5rem;">
                <div style="flex:1; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.3); border-radius:var(--radius-md); padding:1rem; text-align:center;">
                    <i data-lucide="trending-up" style="color:var(--color-success); width:28px; height:28px; margin-bottom:0.5rem;"></i>
                    <div style="font-weight:700; color:var(--color-success); font-size:1rem;">INGRESO</div>
            ...
            <p style="margin-top:0.5rem; font-size:0.85rem; color:var(--text-muted);">Esta regla se aplica automáticamente — no es necesaria ninguna configuración adicional.</p>
        `;
    }
    ...
    TARGET_COLUMNS.forEach(target => {
        if (target.required) {
            ...
            if (target.key === 'nombre_completo' && appState.combineNames) {
                ...
                // Siempre se consideran mapeados: tienen valor automático o de regla
...
// Mapa de nombres de meses en español e inglés y sus abreviaciones
const MONTHS_MAP = {
...
// Analiza si la columna mapeada para fecha contiene nombres o números de meses
function detectFechaAcumuladoType() {
    ...
    // Revisar hasta 10 filas de datos
    ...
    // Si más de la mitad son nombres/números de mes, clasificar como columna de mes
    if (nonEmptyCount > 0 && (monthMatchCount / nonEmptyCount) >= 0.5) {
        appState.fechaAcumuladoIsMonthName = true;
    }
}

// Convierte un nombre o número de mes a una fecha completa dd/mm/aaaa
function parseMonthNameToDate(rawVal, yearStr, dayRule, rawDay) {
    ...
    if (dayRule === 'none' || dayRule === 'no_aplica') {
        if (rawDay !== undefined && rawDay !== null && String(rawDay).trim() !== '') {
            // Limpiar y extraer el número de día
...
// ==========================================
// PASO 3: VALIDACIÓN Y DECISIONES DE CONFIGURACIÓN
// ==========================================
function validateAndGoToStep3() {
    ...
    TARGET_COLUMNS.forEach(target => {
        if (target.required) {
            if (target.key === 'nombre_completo' && appState.combineNames) {
                if (appState.combineSurnamesList.length === 0 && appState.combineNamesList.length === 0) {
                    missingRequired.push(target.name + ' (Combinación de columnas vacía)');
                }
            } else if (target.key === 'fecha_acumulado' && appState.combineMonthYear) {
                if (!appState.monthColumn || !appState.yearColumn) {
                ...
                // Estos campos siempre tienen valor resuelto automáticamente
                target.key === 'tipo_nomina' ||
                target.key === 'quincena' ||
                target.key === 'naturaleza'
            ) {
                // Siempre válidos: tipo_nomina y quincena usan regla por defecto,
                // naturaleza se calcula por el signo del valor
            } else if (!appState.columnMappings[target.key]) {
    ...
    setTimeout(() => {
        try {
            ...
            renderPreviewUI();
            hideLoading();
            setStep(3); // Ir directamente al Paso 3 (Exportar)
            
            // Si no hay errores críticos, celebrar discretamente
            ...
            console.error(err);
            alert('Ocurrió un error inesperado al procesar los datos de nómina: ' + err.message);
            hideLoading();
        }
    }, 150);
}
...
function transformData() {
    ...
    appState.rawRows.forEach((rawRow, rowIndex) => {
        const rowNum = rowIndex + 2; // Basado en 1-index + Fila de cabecera
        ...
        // 3. Fecha de Acumulado (Formato dd/mm/aaaa)
        let parsedDate;
        if (appState.combineMonthYear) {
            const rawMonth = rawRow[appState.monthColumn];
            const rawYear = rawRow[appState.yearColumn];
        ...
        // 13. Mes Acumulado (Basado en la fecha parsed)
        let mesAcumulado = '';
        if (parsedDate.isValid && parsedDate.dateObject) {
            mesAcumulado = String(parsedDate.dateObject.getMonth() + 1); // 1 = Enero, 2 = Febrero, etc.
        }
        ...
        // Si no se mapeó o no se identificó el valor correcto, calcular por signo de Valor
        ...
        // Guardar índice de fila original para debug
    ...
    // Compilar Advertencias y Errores
    if (missingIdCount > 0) {
        appState.validationErrors.push({
            type: 'danger',
            msg: `Se detectaron ${missingIdCount} filas con el campo "Identificación" vacío. Este campo es obligatorio para el SaaS.`
        });
    }
    if (invalidDatesCount > 0) {
        appState.validationErrors.push({
            type: 'warning',
            msg: `Se detectaron ${invalidDatesCount} fechas que no se pudieron formatear. Por favor revisa la vista previa; las filas afectadas muestran "FECHA INVÁLIDA".`
        });
    }
    if (invalidNumbersCount > 0) {
        appState.validationErrors.push({
            type: 'warning',
            msg: `Se detectaron ${invalidNumbersCount} valores numéricos (en Cantidad o Valor) ilegibles o vacíos. Se procesarán como 0, por favor valida.`
        });
    }
}

// Helper para parsear números robustamente
function parseNumber(val) {
    if (val === undefined || val === null || val === '') return 0;
    if (typeof val === 'number') return val;
    
    // Limpiar string de símbolos de moneda, espacios y comas de miles
    let cleaned = String(val).trim();
    cleaned = cleaned.replace(/[\$\s\€]/g, '');
    
    // Si viene con comas para decimales, ej. "1.500,50" -> cambiar a "1500.50"
    // Caso clásico hispano: comas para decimales y puntos para miles
    const commaIndex = cleaned.lastIndexOf(',');
    const dotIndex = cleaned.lastIndexOf('.');
    
    if (commaIndex > dotIndex) {
        // La coma está después del punto, es separador decimal
        cleaned = cleaned.replace(/\./g, '').replace(',', '.');
    } else if (dotIndex > commaIndex && commaIndex !== -1) {
        // El punto está después de la coma, la coma es separador de miles
        cleaned = cleaned.replace(/,/g, '');
    } else if (commaIndex !== -1 && dotIndex === -1) {
        // Solo hay comas, probablemente sea decimal si es menor a 3 dígitos finales
...
// Helper para parsear fechas robustamente
function parseDate(val) {
    if (val === undefined || val === null || val === '') {
        return { isValid: false, formattedString: 'FECHA INVÁLIDA', dateObject: null };
    }
    
    // 1. Si ya es un objeto Date de Javascript (gracias a XLSX)
    if (val instanceof Date) {
        return formatJSDate(val);
    }
    
    // 2. Si viene como un número serial de Excel (ej. 45049)
    if (typeof val === 'number' && val > 30000 && val < 60000) {
        ...
        return formatJSDate(localDate);
    }
    
    // 3. Limpiar string y procesar formatos comunes de texto
    ...
    if (isoMatch) {
        ...
        // Verificar que no hubo desbordamiento de fecha por parte de JS
        if (isValidDate(dateObj) && dateObj.getDate() === d && dateObj.getMonth() === m) {
            return formatJSDate(dateObj);
        }
    }
    
    // Formato Latinoamericano (dd/mm/aaaa) o Americano (mm/dd/aaaa)
    const latMatch = dateStr.match(/^(\d{1,2})[-/](\d{1,2})[-/](\d{4})/);
    if (latMatch) {
        const val1 = parseInt(latMatch[1]);
        const val2 = parseInt(latMatch[2]);
        const y = parseInt(latMatch[3]);
        
        // Intentar primero como Latinoamericano (DD/MM/YYYY)
        let d = val1;
        let m = val2 - 1;
        
        // Si el mes es > 12 pero el día es <= 12, probablemente sea US (MM/DD/YYYY)
        if (val2 > 12 && val1 <= 12) {
            d = val2;
            m = val1 - 1;
        }
    ...
    // Tratar de dejar que el motor del navegador lo parsee
    const fallbackDate = new Date(dateStr);
    if (isValidDate(fallbackDate)) {
        return formatJSDate(fallbackDate);
    }
...
function formatJSDate(dateObj) {
    ...
    return {
        isValid: true,
        formattedString: `${d}/${m}/${y}`,
        dateObject: dateObj
    };
}

// ==========================================
// RENDERIZADO DE VISTA PREVIA Y VALIDACIÓN
// ==========================================
function renderPreviewUI() {
    ...
    appState.transformedData.forEach(row => {
        if (row.naturaleza === 'INGRESO') {
            ...
            // Sumar valor absoluto si viene negativo en la nómina origen
    ...
    // 2. Renderizar Alertas de Validación
    ...
    if (appState.validationErrors.length > 0) {
        appState.validationErrors.forEach(err => {
            const li = document.createElement('li');
            li.className = err.type; // 'warning' o 'danger'
    ...
    // 3. Renderizar Tabla de Vista Previa (primeras 50 filas)
    ...
    previewRows.forEach(row => {
        ...
        // Identificación (con validación de vacío)
...
// ==========================================
// DESCARGA DEL ARCHIVO EXCEL HOMOLOGADO
// ==========================================
function downloadTransformedExcel() {
    try {
        ...
        // Mapear los objetos internos de appState a filas ordenadas según el header de exportación
        ...
        // Crear matriz combinada con cabecera y filas
        const sheetData = [EXPORT_HEADERS, ...rowsToExport];
        
        // Crear nuevo libro y hoja con SheetJS
        ...
        const colWidths = EXPORT_HEADERS.map((header, colIndex) => {
            ...
            return { wch: Math.min(30, maxLength + 3) }; // Limitar ancho a 30 caracteres
        });
        ...
        // Pequeño retardo y alert de éxito
        setTimeout(() => {
...
// Muestra el modal para sugerir unificaciones automáticas
function showUnificationModal(hasSplitNames, hasSplitMonthYear, callback) {
    ...
    if (hasSplitNames) {
        ...
        optionsHtml += `
            <div class="modal-option-item" id="opt-item-names">
                <input type="checkbox" id="modal-chk-names" class="modal-option-checkbox" checked>
                <div class="modal-option-content">
                    <span class="modal-option-title">Unificar Nombre y Apellido</span>
                    <span class="modal-option-desc">Se detectaron columnas separadas para apellidos (<b>${colsSurnames}</b>) y nombres (<b>${colsNames}</b>). Se unificarán en un solo campo <b>Nombre Completo</b> (Formato: Apellidos Nombres).</span>
                </div>
            </div>
        `;
    }
    ...
    if (hasSplitMonthYear) {
        optionsHtml += `
            <div class="modal-option-item" id="opt-item-dates">
                <input type="checkbox" id="modal-chk-dates" class="modal-option-checkbox" checked>
                <div class="modal-option-content">
                    <span class="modal-option-title">Unificar Mes y Año (Fecha de Acumulado)</span>
                    <span class="modal-option-desc">Se detectó que el mes (columna <b>${appState.suggestedMonthColumn}</b>) y el año (columna <b>${appState.suggestedYearColumn}</b>) vienen separados. Se unificarán para construir la <b>Fecha de Acumulado</b>.</span>
                </div>
            </div>
        `;
    }
    ...
    // Renderizar iconos de Lucide en el modal
    lucide.createIcons();
    
    // Añadir interactividad a las tarjetas de opción (hacer clic en la tarjeta alterna el checkbox)
    if (hasSplitNames) {
    ...
    function closeModal() {
        const modal = document.getElementById('unification-modal-overlay');
        if (modal) {
            modal.remove();
        }
    }
...
// ============================================================
// INTEGRACIÓN CON EL DASHBOARD NETOS (NOMAI)
// ============================================================

const dashboardMonthNames = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];

function convertTransformedDataToDashboard(transformedData) {
    return transformedData.map(r => {
        // Parse date for year
        let anio = 2026;
        ...
        // Month name
        const mesIdx = parseInt(r.mes_acumulado) - 1;
        const mesName = dashboardMonthNames[mesIdx] || "Desconocido";
        
        // Quincena period number
        let pa = null;
        if (r.quincena === '1Q') pa = 1;
        else if (r.quincena === '2Q') pa = 2;
        
        // Concept type (Salarial, etc.)
        let tipo = "SALARIAL";
        ...
        return {
            ...
            a: anio,
            t: tipo,
            tn: r.tipo_nomina ? r.tipo_nomina.toString().toUpperCase().trim() : "N",
...
document.addEventListener('DOMContentLoaded', () => {
    const btnLoadDashboard = document.getElementById('load-to-dashboard-btn');
    if (btnLoadDashboard) {
        btnLoadDashboard.addEventListener('click', () => {
            if (!appState.transformedData || appState.transformedData.length === 0) {
                alert('No hay datos transformados listos para cargar. Por favor completa los pasos.');
                return;
            }
            
            // 1. Convert appState.transformedData to dashboard format
            const dashboardData = convertTransformedDataToDashboard(appState.transformedData);
            
            // 2. Load into state
            const consolidated = aggregateRecords(dashboardData);
            if (consolidated.length === 0) {
                alert('No se encontraron registros de pagos válidos.');
                return;
            }
            ...
            // 5. Success feedback
            alert(`¡Se cargaron con éxito ${consolidated.length} registros del archivo: ${appState.fileName} al Dashboard!`);
